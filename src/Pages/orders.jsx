...
const Orders = () => {
  ...
  const [cancelLoading, setCancelLoading] = useState({}); // track cancel button loading
      ...
      if (response.ok) {
        ...
        setOrders((prevOrders) =>
          prevOrders.map((order) =>
            order._id === orderId
              ? { ...order, status: data.status, trackingDetails: data.trackingDetails }
              : order
          )
        );
      } else {
  ...
  const cancelOrder = async (orderId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://jodiacxthreadorabackend.store/api/orders/cancel/${orderId}`,
        {
                ...
                >
                  ...
                  <div className="p-6 space-y-4">
                    {order.orderItems?.map((item, index) => (
                      <div
                        key={index}
                        className="flex items-center space-x-4 border-b border-gray-100 last:border-b-0 pb-4 last:pb-0"
                      >
                        <img
                    ...
                    <div className="flex flex-col items-end gap-2 text-right">
                      ...
                      {(order.status === "Pending" || order.status === "Processing") && (
                        <button
                          ...
                          className="w-full sm:w-auto px-6 py-3 text-sm font-semibold rounded-full text-white bg-red-600 hover:bg-red-700 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                      ...
                      {order.trackingDetails && (
                        <p className="text-xs text-gray-500 mt-2">
                          ...
                          <span className="font-semibold text-gray-700">
                            {order.trackingDetails?.current_status || "N/A"}
                          </span>
