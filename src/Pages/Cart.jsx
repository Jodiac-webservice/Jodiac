...
const Cart = () => {
  ...
  async function fetchCart() {
    ...
    try {
      const res = await fetch("https://jodiacxthreadorabackend.store/api/cart", {
        method: "GET",
        headers: {
      ...
      setError("An error occurred while fetching the cart",err);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleRemoveItem(productId) {
    ...
    try {
      const res = await fetch("https://jodiacxthreadorabackend.store/api/cart/remove", {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        ...
        {cart?.products?.map((item) => (
          ...
          >
            ...
            <motion.img
              ...
              onClick={() => navigate(`/productPage/${item.productId._id}`)}
            />
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              {item.productId.oldPrice && (
                <p className="text-gray-500 line-through text-sm">₹{item.productId.oldPrice}</p>
              )}
            </div>
